...
export class RawSigner {
  ...
  async signMessage(typed: any, accountAddress: string): Promise<[string, string]> {
    const messageHash = typedData.getMessageHash(typed, accountAddress)
    return this.signRaw(messageHash)
  }

  async signTransaction(transactions: any, details: any): Promise<[string, string]> {
    ...
    } else if (Object.values(RPC.ETransactionVersion3).includes(details.version)) {
      msgHash = hash.calculateInvokeTransactionHash({
        ...details,
        senderAddress: details.walletAddress,
        compiledCalldata,
  ...
  async signDeclareTransaction(details: any): Promise<[string, string]> {
    let msgHash: string
    if (Object.values(RPC.ETransactionVersion2).includes(details.version)) {
      msgHash = hash.calculateDeclareTransactionHash({ ...details, version: details.version })
    } else if (Object.values(RPC.ETransactionVersion3).includes(details.version)) {
